...
func TestNewDepositSetup(t *testing.T) {
	s := test.NewSetup(t)
	...
	outpConclude, err := chanconn.Conclude(nonceArray, params.Parts, params.ChallengeDuration, chanId, state.Version, &alloc, statefinal, sigs, *recipPerunID, execPathTyped)
	require.NoError(t, err)
